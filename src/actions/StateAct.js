...
export const addState = state => {
  return dispatch => {
    dispatch({ type: STATE_LOAD });
    return axios
      .post(`${RU}/state/add`, state, { headers: { sabti: localStorage.getItem("token") } })
...
export const addTownToState = town => {
  return dispatch => {
    ...
    return axios
      .post(`${RU}/state/add/town`, town, { headers: { sabti: localStorage.getItem("token") } })
...
export const updateState = state => {
  return dispatch => {
    ...
    return axios
      .post(`${RU}/state/update`, state, { headers: { sabti: localStorage.getItem("token") } })
      .then(resp => {
