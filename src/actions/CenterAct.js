...
import {
  ...
  SET_EXPERT_RATE_LOADING,
  SET_EXPERT_RATE_SET,
  SET_EXPERT_RATE_ERR,
  SET_OPTION_FOR_CENTER,
  SET_OPTION_FOR_CENTER_LOADING,
  DOCTOR_SET_LOAD,
  DOCTOR_SET,
  DOCTOR_SET_ERR,
  OWNER_SET_LOAD,
  ...
  GET_EDITED_CENTER,
  ERR_EDITED_CENTER,
  SET_CENTER_LIKE_LOAD,
  SET_CENTER_LIKE,
  SET_OTHER_ADDRESS_LOAD,
  SET_OTHER_ADDRESS,
  SET_OTHER_ADDRESS_ERR,
  CENTER_UPDATE,
  REMOVE_CENTER_ODD_ADD_LOADING,
  REMOVE_CENTER_ODD_ADD_END,
  CENTER_LICENSE_PIC_UPLOAD_LOAD,
  CENTER_LICENSE_PIC_UPLOAD,
  RU
} from "./types";
...
export function getCenterById(id) {
  return function(dispatch) {
    ...
    return axios
      .get(`${RU}/center`, { params: { id: id }, headers: { sabti: localStorage.getItem("token") } })
      .then(resp => {
        return dispatch({ type: GET_CENTER, payload: resp.data.center });
      })
      .catch(e => {
        return dispatch({ type: GET_CENTER_ERR });
      });
  };
}
...
export function changeQualityRate(rating, id) {
  return function(dispatch) {
    ...
    return axios
      .post(
        `${RU}/center/addqualityrate`,
        { qualityRate: rating, centerId: id },
        {
...
export function changeSalesmanRate(rating, id) {
  return function(dispatch) {
    ...
    return axios
      .post(
        `${RU}/center/addsalesmanrate`,
        { salesmanRate: rating, centerId: id },
        {
          headers: { sabti: localStorage.getItem("token") }
        }
      )
...
export function changePriceRate(rating, id) {
  return function(dispatch) {
    ...
    return axios
      .post(
        `${RU}/center/addpricerate`,
        { priceRate: rating, centerId: id },
        {
          headers: { sabti: localStorage.getItem("token") }
        }
      )
...
export function removeOtherAddressFromCenter(centerId, addId) {
  return function(dispatch) {
    ...
    return axios
      .post(`${RU}/center/remove/other/address`, { centerId, addId }, { headers: { sabti: localStorage.getItem("token") } })
      .then(resp => {
        dispatch({ type: GET_CENTER, payload: resp.data.center });
...
export const setOtherAddressForCenter = address => {
  return dispatch => {
    ...
    return axios
      .post(`${RU}/center/set/address`, address, {
        headers: { sabti: localStorage.getItem("token") }
      })
      .then(resp => {
        return dispatch({ type: GET_CENTER, payload: resp.data.center });
...
export const setOfficeDoctorsForCenter = (officeDoctors, centerId) => {
  return dispatch => {
    ...
    return axios
      .post(
        `${RU}/center/set/officedoctors`,
        { officeDoctors, centerId },
        {
          headers: { sabti: localStorage.getItem("token") }
        }
      )
...
export const getEditedCenter = centerId => {
  return dispatch => {
    ...
    return axios
      .get(`${RU}/center/edited`, { params: { centerId }, headers: { sabti: localStorage.getItem("token") } })
      .then(resp => {
        return dispatch({ type: GET_EDITED_CENTER, payload: resp.data.center });
      })
      .catch(e => {
        return dispatch({ type: ERR_EDITED_CENTER });
      });
  };
};
...
export const removeCenterPic = (_id, pic) => {
  return dispatch => {
    dispatch({ type: LOAD_EDITED_CENTER });
    return axios
      .post(`${RU}/center/delete/pic`, { _id, pic }, { headers: { sabti: localStorage.getItem("token") } })
...
export const licensePicUpload = ({ file }) => {
  return dispatch => {
    dispatch({ type: CENTER_LICENSE_PIC_UPLOAD_LOAD });
    ...
    return axios
      ...
      .then(resp => {
        dispatch(change("AddBusinessLicenseModal", "licensePicRef", resp.data._id));
        dispatch(change("AddBusinessLicenseModal", "licensePic", resp.data.name));
        return dispatch({ type: CENTER_LICENSE_PIC_UPLOAD, payload: resp.data });
      })
      .catch(error => {});
  };
};
