...
import { GET_CENTERS, ADD_CENTER, REMOVE_CENTER, CENTERS_LOAD, YOUR_CENTER, ADD_CENTER_ERR,
				YOUR_CITY, CENTER_PIC_LOAD, REMOVE_CENTER_ERR, CLEAN_CENTERS,
				CENTER_UPDATE_LOAD, CENTER_UPDATE, CENTER_UPDATE_ERR,
				PIC_UPLOAD_PERCENT, PICS_UPLOADED, CENTER_PIC_LOAD_DONE, CLEAN_PIC_UPLOAD_PERCENT,
				SET_CENTER_TO_POPUP, SET_POPUP_TO_NULL,
				GET_CENTERS_COUNT_LOAD, GET_CENTERS_COUNT, GET_CENTERS_COUNT_ERR,
				RU } from './types';

export function getCenterWithQuery(query) {
	return function ( dispatch ) {
    dispatch( { type: CENTERS_LOAD } )
		return axios.get( `${ RU }/centers/params`, {
			params: query
		} ).then( resp => {
...
export function getCenters(query) {
	return function ( dispatch ) {
    dispatch( { type: CENTERS_LOAD } )
		return axios.get( `${ RU }/centers`, { params: query } )
			.then( resp => {
				dispatch( { type: YOUR_CITY, payload: resp.data.city } )
				return dispatch( { type: GET_CENTERS, payload: resp.data.centers } )
			} )
...
export const yourCenter = (centerid) => {
	return function ( dispatch ) {
    dispatch( { type: CENTERS_LOAD } )
		return axios.get( `${ RU }/yourcenter`, {
      params: { centerid },
...
export function addCenter( center ) {
	return function ( dispatch ) {
    dispatch( { type: CENTERS_LOAD } )
		return axios.post( `${ RU }/center/add`, center, {
			headers: { sabti: localStorage.getItem( 'token' ) }
		} ).then( resp => {
	    return dispatch( { type: ADD_CENTER, payload: resp.data.center } );
		} ).catch( ( error ) => {
...
export function updateCenter( center ) {
	return function ( dispatch ) {
    dispatch( { type: CENTERS_LOAD } )
		return axios.post( `${ RU }/center/edit`, center, {
			headers: { sabti: localStorage.getItem( 'token' ) }
		} ).then( resp => {
...
export function removeCenter( id ) {
	return function ( dispatch ) {
    dispatch( { type: CENTERS_LOAD } )
		return axios.post( `${ RU }/center/remove`, { id }, {
			headers: { sabti: localStorage.getItem( 'token' ) }
		} ).then( resp => {
...
export const centerChangePic = ({ file, id }) => {
    ...
    dispatch({type: CENTER_UPDATE_LOAD })
		...
		return axios.put( `${ RU }/change/center/pic`, data, config )
			.then(resp => {
				return dispatch( { type: CENTER_UPDATE, payload: resp.data.center } )
			}).catch(error => {
...
export const addPicToCenter = (center) =>  {
	return ( dispatch ) => {
    dispatch({type: CENTER_UPDATE_LOAD })
		return axios.post( `${ RU }/center/add/pic`, center, {
			headers: { sabti: localStorage.getItem( 'token' ) }
		} )
			.then(resp => {
				return dispatch( { type: CENTER_UPDATE, payload: resp.data.center } )
...
export const GetCentersCount = ( ) => {
    ...
    dispatch( { type: GET_CENTERS_COUNT_LOAD } )
		return axios.get( `${ RU }/center/get/count`, { headers: { sabti: localStorage.getItem( 'token' ) } } )
