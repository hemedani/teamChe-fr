...
import {
  ...
  GET_OWN_USER_LOAD,
  GET_OWN_USERS,
  ADD_ADDRESS_TO_USER_LOAD,
  UPDATE_USER_ADDRESS,
  SHOW_ADD_ADDRESS_MODAL,
  HIDE_ADD_ADDRESS_MODAL,
  REMOVE_USER_ADDRESS_LOAD,
  GET_USERS_COUNT_LOAD,
  GET_USERS_COUNT,
  GET_USERS_COUNT_ERR,
  UPDATE_USER_ERR,
  USER_UPDATE_LOAD,
...
export function removeUser({ _id }) {
  return function(dispatch) {
    ...
    return axios
      .post(`${RU}/user/remove`, { _id }, { headers: { sabti: localStorage.getItem("token") } })
...
export function userChangePic({ file, id }) {
  return function(dispatch) {
    ...
    return axios
      .put(`${RU}/change/user/pic`, data, config)
...
export const getUsers = (query = {}) => {
  return dispatch => {
    ...
    return axios
      .get(`${RU}/users`, { params: query, headers: { sabti: localStorage.getItem("token") } })
...
export function getUsersWithQuery({ email, familyName, phone }) {
  return function(dispatch) {
    ...
    dispatch({ type: CLEAN_USERS });
    return axios
      .post(`${RU}/users/withsearch`, { email, familyName, phone }, { headers: { sabti: localStorage.getItem("token") } })
      .then(resp => {
...
export function getDoctorUser() {
  return function(dispatch) {
    ...
    dispatch({ type: CLEAN_USERS });
    return axios
      .get(`${RU}/users/get/doctor`, { headers: { sabti: localStorage.getItem("token") } })
      .then(resp => dispatch({ type: GET_USERS, payload: resp.data.users }))
...
export function getUserWithLevel(level) {
  return function(dispatch) {
    ...
    dispatch({ type: CLEAN_USERS });
    return axios
      .get(`${RU}/users/withlevel`, { params: { level }, headers: { sabti: localStorage.getItem("token") } })
      .then(resp => {
        return dispatch({ type: GET_USERS, payload: resp.data.users });
      })
      .catch(e => {
...
export const updateUser = ({ _id, name, familyName, expertise, address, doctor, level }) => {
  return dispatch => {
    ...
    return axios
      .post(
        `${RU}/user/edit`,
        { _id, name, familyName, expertise, address, doctor, level },
        {
          headers: { sabti: localStorage.getItem("token") }
        }
      )
...
export const addAddressToUser = Address => {
  return dispatch => {
    ...
    return axios
      .post(`${RU}/user/add/address`, Address, { headers: { sabti: localStorage.getItem("token") } })
      .then(resp => {
...
export const removeUserAddress = _id => {
  return dispatch => {
    ...
    return axios
      .post(`${RU}/user/remove/address`, { _id }, { headers: { sabti: localStorage.getItem("token") } })
...
export const GetUsersCount = () => {
  return dispatch => {
    ...
    return axios
      .get(`${RU}/user/get/count`, { headers: { sabti: localStorage.getItem("token") } })
