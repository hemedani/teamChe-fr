...
import { 
	GET_RATE, GET_NEXT_RATE, RATE_LOAD, GET_YOUR_RATE, ACCEPTED_RATE, ACCEPT_RATE_LOAD, DENIED_RATE,
	WARE_RATE_LOAD, GET_WARE_RATE, TEXT_RATE_LOAD, CHANGE_YOUR_WARE_RATE, CHANGE_WARE_RATE_LOADING,
	SET_REPLY_LOAD, SET_REPLY, SET_REPLY_ERR, GET_YOUR_WARE_RATE_ERR,
	CLEAN_WARE_RATES, CLEAN_YOUR_WARE_RATE, CLEAN_RATE, ADD_VOTE_LOAD, UP_RATE_IN_RATES, UP_VOTE_MASSAGE, RU } from './types';
...
export const getWareRating = (wareId, rateId) => {
    ...
    dispatch( { type: WARE_RATE_LOAD } )
		return axios.get( `${ RU }/rates/ware`, { params: {wareId: wareId, rateId: rateId}, } )
...
export const getNotAcceptedRates = (query) => {
	return ( dispatch ) => {
    dispatch( { type: RATE_LOAD } )
		return axios.get( `${ RU }/rates/notaccepted`, { params: query, headers: { sabti: localStorage.getItem( 'token' ) }} )
			.then( resp => {
				return dispatch( { type: GET_RATE, payload: resp.data.rates } )
			} )
			.catch( ( e ) => {
...
export const getYourRate = (centerId) => {
	return ( dispatch ) => {
    dispatch( { type: RATE_LOAD } )
		return axios.get( `${ RU }/rates/yourrate`, {
...
export const sendRateText = ({ yourRateId, textRate, centerId }) => {
    ...
    dispatch( { type: TEXT_RATE_LOAD} )
		return axios.post( `${ RU }/rates/addtext`, { rateId: yourRateId, rateText: textRate, centerId }, {
			headers: { sabti: localStorage.getItem( 'token' ) }
		} ).then( resp => {
...
export const sendVote = ({ rateId, plus, minus }) => {
    ...
    dispatch( { type: ADD_VOTE_LOAD } )
		return axios.post( `${ RU }/vote/add`, { rateId: rateId, plus, minus }, {
			headers: { sabti: localStorage.getItem( 'token' ) }
		} ).then( resp => {
...
export const sendWareRateText = ( rateId, rateText, wareId ) => {
    ...
    dispatch( { type: CHANGE_WARE_RATE_LOADING } )
		return axios.post( `${ RU }/rates/add/ware/text`, { rateId, rateText, wareId }, {
			headers: { sabti: localStorage.getItem( 'token' ) }
		} ).then( resp => {
			toastr.success('با تشکر', 'نظر شما با موفقیت ثبت شد')
...
export const deniedRate = ({ rateId }) => {
    ...
    dispatch( { type: ACCEPT_RATE_LOAD, payload: rateId } )
		return axios.post( `${ RU }/rates/deniedRate`, { rateId }, {
			headers: { sabti: localStorage.getItem( 'token' ) }
		} ).then( resp => {
...
export const justDeniedRate = ({ rateId }) => {
	return ( dispatch ) => {
    dispatch( { type: ACCEPT_RATE_LOAD, payload: rateId } )
		return axios.post( `${ RU }/rates/just/denied`, { rateId }, {
			headers: { sabti: localStorage.getItem( 'token' ) }
		} )
...
export const acceptRate = ({ rateId }) => {
	return ( dispatch ) => {
    dispatch( { type: ACCEPT_RATE_LOAD, payload: rateId } )
		return axios.post( `${ RU }/rates/acceptRate`, { rateId }, {
			headers: { sabti: localStorage.getItem( 'token' ) }
		} ).then( resp => {
...
export const setReply = (rate) => {
    ...
    dispatch( { type: SET_REPLY_LOAD } )
		return axios.post( `${ RU }/rates/set/reply`, rate, { headers: { sabti: localStorage.getItem( 'token' ) } } )
...
export const getYourWareRate = (wareId) => {
	return ( dispatch ) => {
    dispatch( { type: CHANGE_WARE_RATE_LOADING } )
		return axios.get( `${ RU }/rates/your/ware/rate`, { headers: { sabti: localStorage.getItem( 'token' ) }, params: { wareId } } )
