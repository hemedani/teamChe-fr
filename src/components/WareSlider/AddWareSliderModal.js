...
import { addWareSlider, wareSliderUploadPic, ADD_WARESLIDER } from '../../actions'
import _ from 'lodash'
import Loader from '../Utils/Loader'
import { RenderField, required } from '../Utils/FormField';
class AddWareSliderModal extends Component {
  constructor(props) {
    super(props)
    this.state = {
      file: '',
  ...
  handleSubmitPic(e) {
    ...
    this.props.wareSliderUploadPic({ file })
  ...
  onSubmitForm(inp) {
    this.props.addWareSlider(inp)
      .then((resp) => {
        if (resp.type === ADD_WARESLIDER ) { this.props.history.push('/manage/wareSliders'); }
      });
  }
  render() {
        ...
        <div className='modal'>
          ...
          <form onSubmit={this.handleSubmitPic}>
            ...
            <div className='chapchin width-same'>
              {this.props.wareSliders.picLoading ? ( <div className='vorod-bargozari'> <Loader /> </div> ) :
...
AddWareSliderModal = reduxForm({ form: 'AddWareSliderModal' })(AddWareSliderModal);

const mps = ({ wareSliders }) => ({ wareSliders });

export default connect(mps, { addWareSlider, wareSliderUploadPic })(AddWareSliderModal);
