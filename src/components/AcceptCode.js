...
class AcceptCode extends Component {
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      if (this.props.auth.authTimer <= 1) {
  ...
  onSubmitForm() {
    const { phone } = this.props.auth.user;
    const { code } = this.state;
    // console.log(id)
    this.props.sendCode({ code, phone }).then(resp => {
      ...
      <div className="ghab-vorod grid">
        ...
        <div className="poshtzamine">
            ...
            <input
              ...
              onChange={e => {
                if (e.target.value.length > 4) {
                  ...
                  this.setState({ code: e.target.value, err: null });
                }
              }}
            />
          ...
          <div className="auth-timer" style={{ with: "100%", textAlign: "center" }}>
            <div>لطفا کد اس ام اس شده را ارسال کنید</div>
            <span>{hhmmss(auth.authTimer)}</span>
          </div>

          {auth.error && <div className="auth-error">مشکلی در ورود یا ثبت نام به وجود آمده لطفا دوباره تلاش کنید</div>}
          ...
          {auth.loginLoading ? (
            ...
            <div className="chapchin width-same">
              <button
                disabled={!this.state.code || this.state.code.length !== 4}
                className="dogme i-round i-sabz"
