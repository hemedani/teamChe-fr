...
export const minValue = min => value => (value && value < min ? `باید حداقل ${min} کاراکتر باشد` : undefined);
export const minValue18 = minValue(18);
export const email = value =>
  value && !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(value) ? "این ایمیل صحیح نیست" : undefined;
export const tooOld = value => (value && value > 80 ? "خیلی سنت زیاده حاجی" : undefined);
...
export const phoneMobile = number => {
  ...
  if (
    (_.startsWith(number, 989) && number.toString().length === 12) ||
    (_.startsWith(number, 9) && number.toString().length === 10)
  ) {
    return undefined;
  } else {
