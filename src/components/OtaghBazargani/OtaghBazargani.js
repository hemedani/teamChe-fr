...
import { removeOtaghBazargani, RU } from "../../actions";

class OtaghBazargani extends Component {
  directPic() {
    if (this.props.pic) {
  ...
  remove(id) {
    ...
    this.props.removeOtaghBazargani(id);
  }
  render() {
    return (
        ...
        <div className="grid-manage-detail">
          ...
          <div className="grid-manage-btn">
            <Link
              ...
              to={{ pathname: `/manage/otaghBazargani/edit/${this.props._id}` }}
            >
              {" "}
              ویرایش{" "}
            </Link>
            <Link className="dogme i-round i-abi round-small" to={`/manage/otaghBazargani/changepic/${this.props._id}`}>
              {" "}
              تعویض تصویر{" "}
            </Link>
