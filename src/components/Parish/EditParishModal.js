...
import { updateParish, UPDATE_PARISH } from "../../actions";
import DotLoader from "../Utils/DotLoader";
import { RenderField, required } from "../Utils/FormField";
import Map from "../Utils/MapBox";

class EditParishModal extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    this.props.dispatch(change("EditParishModal", "lat", initialValues.location.coordinates[1]));
    this.props.dispatch(change("EditParishModal", "lng", initialValues.location.coordinates[0]));
  }
  ...
  onSubmitForm(v) {
    ...
    if (polygon) {
      ...
      this.props.updateParish({ ...v, polygon }).then(resp => {
        if (resp.type === UPDATE_PARISH) this.props.history.goBack();
      });
    } else {
  ...
  onDragEnd(e) {
    this.props.dispatch(change("EditParishModal", "lat", e.getLatLng().lat));
    this.props.dispatch(change("EditParishModal", "lng", e.getLatLng().lng));
  }
...
EditParishModal = reduxForm({
  form: "EditParishModal",
  validate
})(EditParishModal);

const msp = ({ parishes }, props) => {
  const parish = _.find(parishes.parishes, cnt => cnt._id === props.match.params.id);
  return { initialValues: parish };
};
