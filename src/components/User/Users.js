...
class Users extends Component {
  ...
  bigiHame() {
    ...
    let _id = users.length >= 1 ? users[users.length - 1]._id : null;
    this.props.getUsers({ _id });
  }
  ...
  beparRanande(noe) {
    this.props.getUserWithLevel(noe);
  }
          ...
          <div className="poshtzamine">
            ...
            {userLoading ? (
              ...
              <div className="chapchin width-same-big">
                <span onClick={() => this.props.getUsers()} className="dogme i-round i-abi">
                  همه کاربران
                </span>
                <span onClick={this.beparRanande.bind(this, "doctor")} className="dogme i-round i-abi">
                  پزشکان
                </span>
                <span onClick={this.beparRanande.bind(this, "editor")} className="dogme i-round i-abi">
                  ویرایشگران
                </span>
                <span onClick={this.beparRanande.bind(this, "expert")} className="dogme i-round i-abi">
                  کارشناسان
                </span>
                <Link to="/manage/users/add" className="dogme i-round i-abi">
                  ثبت نام
                </Link>
              </div>
            )}
            ...
            <div className="grid-section end-aligned">
              ...
              {this.props.users.countLoading ? (
                <DotLoader height="2.5rem" width="6rem" />
              ) : (
                <span className="dogme i-sabz i-round" onClick={() => this.props.GetUsersCount()}>
                  دریافت
                </span>
              )}
            </div>
            ...
            {users.length >= 1 && (
              <div className="grid-section">
                {users.map((user, i) => (
                  <User key={i} {...user} remove={this.remove.bind(this)} />
                ))}
              </div>
            )}
            {userLoading ? (
              ...
              <div className="chapchin width-same-big">
                <span className="dogme i-round i-abi" onClick={this.bigiHame.bind(this)}>
                  ادامه
                </span>
              </div>
            )}
...
export default connect(
  mps,
  { getUsers, getUsersWithQuery, getUserWithLevel, removeUser, GetUsersCount }
)(Users);
