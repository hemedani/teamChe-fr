...
class LoginWithMob extends Component {
  ...
  onSubmitForm({ phone }) {
    ...
    this.props.signWithMob({ phone, captcha }).then(resp => {
        ...
        <div className="poshtzamine">
          <form onSubmit={handleSubmit(this.onSubmitForm.bind(this))}>
            <Field
              ...
              type="number"
              tak={true}
              validate={[required, number, phoneMobile]}
            />

            {auth.error && (
              <div className="auth-error">مشکلی در ورود یا ثبت نام به وجود آمده لطفا چند دقیقه بعد دوباره تلاش کنید</div>
            )}

            <div className="recaptcha">
              <ReCAPTCHA
                ref="recaptcha"
                sitekey="6LdEIZ8UAAAAAPD9h5Shtf2N8wpLM2PCLrGOJnyE"
            ...
            {auth.loginLoading ? (
              ...
              <div className="chapchin width-same">
                <button
                  type="submit"
                  disabled={invalid || submitting || pristine || !this.state.captcha}
