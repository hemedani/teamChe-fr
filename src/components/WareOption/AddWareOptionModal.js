...
import { addWareOption, wareOptionUploadPic, ADD_WARE_OPTION} from '../../actions'
import _ from 'lodash'
import Loader from '../Utils/Loader'


class AddWareOptionModal extends Component {
  constructor(props) {
    super(props)
    this.state = {
      file: '',
  ...
  handleSubmitPic(e) {
    ...
    this.props.wareOptionUploadPic({ file })
  ...
  onSubmitForm({ name, enName, pic, picRef }) {
    this.props.addWareOption({ name, enName, pic, picRef })
      .then((resp) => {
        if (resp.type === ADD_WARE_OPTION) { this.props.history.push('/manage/wareOptions'); }
      });
  }
        ...
        <div className='modal'>
          ...
          <form onSubmit={this.handleSubmitPic}>
            ...
            <div className='chapchin width-same'>
              {this.props.wareOptions.picLoading ? ( <div className='vorod-bargozari'> <Loader /> </div> ) :
...
AddWareOptionModal = reduxForm({ form: 'AddWareOptionModal', validate })(AddWareOptionModal);

const mps = ({ wareOptions }) => ({ wareOptions });

export default connect(mps, { addWareOption, wareOptionUploadPic })(AddWareOptionModal);
