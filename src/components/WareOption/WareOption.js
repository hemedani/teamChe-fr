...
import { removeWareOption, RU } from '../../actions';

class WareOption extends Component {
	directPic() {
		if ( this.props.pic ) {
  ...
  remove(id) {
    this.props.removeWareOption(id);
  }
  render() {
  	return (
			<div className='grid-manage'>
				...
				<div className="grid-manage-detail">
					...
					<div className="grid-manage-btn">
						<Link className='dogme i-round i-sabz round-small' to={{ pathname: `/manage/wareOptions/edit/${ this.props._id }` }}> ویرایش </Link>
						<Link className='dogme i-round i-abi round-small' to={`/manage/wareOptions/changepic/${ this.props._id }`}> تعویض تصویر </Link>
...
export default connect(null, { removeWareOption })(WareOption);
